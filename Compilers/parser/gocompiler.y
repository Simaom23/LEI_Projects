...
%token RESERVED
%token<str>ID STRLIT INTLIT REALLIT
...
VarSpecList: COMMA ID VarSpecList                                                   {$$=insert_varspec_list($3, $2);}
    | Type                                                                          {$$=insert_varspec_list(NULL, $1);}
    ;
...
Statement: ID ASSIGN Expr                                                           {$$=insert_idexpr($1, $3);}
    | LBRACE StatementList1 RBRACE                                                  {$$=insert_pstatementlist($2);}
    | IF Expr LBRACE StatementList RBRACE                                           {$$=insert_if($2, $4, NULL);}
    | IF Expr LBRACE StatementList RBRACE ELSE LBRACE StatementList RBRACE          {$$=insert_if($2, $4, $8);}
    | FOR LBRACE StatementList RBRACE                                               {$$=insert_for(NULL, $3);}
    | FOR Expr LBRACE StatementList RBRACE                                          {$$=insert_for($2, $4);}
    | RETURN                                                                        {$$=insert_return(NULL);}
    | RETURN Expr                                                                   {$$=insert_return($2);}
    | FuncInvocation                                                                {$$=insert_pfuncinvocation($1);}
    | ParseArgs                                                                     {$$=insert_pparseargs($1);}
    | PRINT LPAR Expr RPAR                                                          {$$=insert_print($3, NULL);}
    | PRINT LPAR STRLIT RPAR                                                        {$$=insert_print(NULL, $3);}
    | error                                                                         {;}
    ;
...
ParseArgs: ID COMMA BLANKID ASSIGN PARSEINT LPAR CMDARGS LSQ Expr RSQ RPAR          {$$=insert_parseargs($1, $9);}
...
Expr: Expr OR Expr                                                                  {$$=insert_expr_operator($1, "Or", $3);}
    | Expr AND Expr                                                                 {$$=insert_expr_operator($1, "And", $3);}
    | Expr LT Expr                                                                  {$$=insert_expr_operator($1, "Lt", $3);}
    | Expr GT Expr                                                                  {$$=insert_expr_operator($1, "Gt", $3);}
    | Expr EQ Expr                                                                  {$$=insert_expr_operator($1, "Eq", $3);}
    | Expr NE Expr                                                                  {$$=insert_expr_operator($1, "Ne", $3);}
    | Expr LE Expr                                                                  {$$=insert_expr_operator($1, "Le", $3);}
    | Expr GE Expr                                                                  {$$=insert_expr_operator($1, "Ge", $3);}
    | Expr PLUS Expr                                                                {$$=insert_expr_operator($1, "Add", $3);}
    | Expr MINUS Expr                                                               {$$=insert_expr_operator($1, "Sub", $3);}
    | Expr STAR Expr                                                                {$$=insert_expr_operator($1, "Mul", $3);}
    | Expr DIV Expr                                                                 {$$=insert_expr_operator($1,"Div", $3);}
    | Expr MOD Expr                                                                 {$$=insert_expr_operator($1, "Mod", $3);}
    | NOT Expr                                                                      {$$=insert_expr_signal($2, "Not");}
    | MINUS Expr %prec DIV                                                          {$$=insert_expr_signal($2, "Minus");}
    | PLUS Expr %prec DIV                                                           {$$=insert_expr_signal($2, "Plus");}
