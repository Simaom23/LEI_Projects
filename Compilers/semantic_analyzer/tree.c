...
void print_idexpr(p_id_expr *pie)
{
    ...
    if (annotated)
    {
        ...
        printf("Assign - ");
        if(token && token->id && token->id->type) print_annotated_type(token->id->type);
        else printf("undef\n");
    }
    else
        printf("Assign\n");
    ...
    if (annotated)
    {
        if (token && token->id && token->id->id) printf("Id(%s) - ", token->id->id);
        //else printf("Id(undef) - ");
        if (token && token->id && token->id->type) print_annotated_type(token->id->type);
        else printf("undef\n");
    }
    else
...
void print_funcinvocation(is_funcinvocation *ifi)
{
    ...
    if (annotated)
    {
        ...
        if (token && token->funcdec && token->funcdec->type && strcmp(token->funcdec->type, "none"))
        {
            printf("Call - ");
    ...
    if (annotated)
    {
        if (token && token->funcdec && token->funcdec->id) printf("Id(%s) - (", token->funcdec->id);
        else printf("undef - (");
        if (token && token->funcdec && token->funcdec->params) print_params(token->funcdec->params);
        else print_typeexprlist(ifi->u_exprlist);
        printf(")\n");
    }
    else
        printf("Id(%s)\n", ifi->id->id);
...
// ParseArgs
//TODO: Verificar se Id e expr têm o mesmo type
void print_parseargs(is_parseargs *ipa)
{
    ...
    const char *type = print_typeexpr(ipa->u_expr);
    ...
    if (annotated)
    {
        ...
        printf("ParseArgs - ");
        if (token && token->id && token->id->type && strcmp(token->id->type, type)==0) print_annotated_type(token->id->type);
        else printf("undef\n");
    }
    else
        printf("ParseArgs\n");
        ...
        if (!strcmp(io->operator->id, "Plus") || !strcmp(io->operator->id, "Minus") || !strcmp(io->operator->id, "Mul") || !strcmp(io->operator->id, "Div") || !strcmp(io->operator->id, "Mod"))
        {
            ...
            while (1)
            {
                ...
                case d_literal:
                {
                    ...
                    if (token_search && token_search->id && !strcmp(token_search->id, "Id"))
                    {
                    ...
                    else if (token_search && token_search->id && !strcmp(token_search->id, "IntLit"))
                        type1 = "int";
                    else if (token_search && token_search->id && !strcmp(token_search->id, "RealLit"))
                        type1 = "float32";
                    else type1 = strdup("undef");
                    break;
                }
                case d_expr_funcinvocation:
                {
                    ...
                    if (token && token->funcdec && token->funcdec->type) type1 = token->funcdec->type;
                    else type1 = strdup("undef");
                    break;
                }
                case d_expr1:
            ...
            while (1)
            {
                ...
                case d_expr_funcinvocation:
                {
                    is_funcinvocation *func_search = search->expr.u_funcinvocation->u_funcinvocation;
                    table_element *token = search_funcdecl(globalSymtab, func_search->id->id);
                    if (token && token->funcdec && token->funcdec->type) type2 = token->funcdec->type;
                    else type2 = strdup("undef");
                    break;
                }
                case d_expr1:
        ...
        while (1)
        {
            ...
            case d_literal:
            {
                ...
                if (token_search && token_search->id && !strcmp(token_search->id, "Id"))
                {
                    ...
                    if (token == NULL)
                        token = search_id(globalSymtab, token_search->value->id, false);
                    if (token && token->id && token->id->type) type = token->id->type;
                    else type = strdup("undef");
                }
                else if (token_search && token_search->id && !strcmp(token_search->id, "IntLit"))
                    type = "int";
                else if (token_search && token_search->id && !strcmp(token_search->id, "RealLit"))
            ...
            case d_expr_funcinvocation:
            {
                is_funcinvocation *func_search = search->expr.u_funcinvocation->u_funcinvocation;
                table_element *token = search_funcdecl(globalSymtab, func_search->id->id);
                if (token && token->funcdec && token->funcdec->type) type = token->funcdec->type;
                else type = strdup("undef");
                break;
            }
            case d_expr1:
...
void print_typeexprlist(is_exprlist *ie)
{
    if (ie == NULL)
        return;

    printf("%s", print_typeexpr(ie->u_expr));

    for (ie = ie->next; ie != NULL; ie = ie->next)
    {
        printf(", %s", print_typeexpr(ie->u_expr));
    }
}

const char* print_typeexpr(is_expr *ie)
{
    is_expr *search = ie;
    char *type = NULL;
    while (1)
    {
        ...
        {
        case d_oper:
            if (strcmp(search->expr.u_oper->operator->id, "Mod") && strcmp(search->expr.u_oper->operator->id, "Div") && strcmp(search->expr.u_oper->operator->id, "Star") && strcmp(search->expr.u_oper->operator->id, "Minus") && strcmp(search->expr.u_oper->operator->id, "Plus"))
                type = (char*)strdup("bool");
            else 
                search = search->expr.u_oper->u_expr_left;
            break;
